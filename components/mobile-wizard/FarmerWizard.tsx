...
import WizardRail from '@/components/mobile-wizard/WizardRail'
...
/**
 * The multi-step Farmer Portal wizard, shared by two routes:
 *  - `/mobile-wizard` — the public, farmer-facing portal (standalone chrome).
 *  - `/dashboard/farmer-form` — the same form embedded in the LGU dashboard
 *    shell (`embedded`), where the admin sidebar is the navigation. In that
 *    mode the public "Balik sa AgriPulse" exit and the farmer bottom tabs are
 *    hidden so the admin stays inside the dashboard.
 */
export default function FarmerWizard({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <main style={{ background: '#F8F7F2', minHeight: '100vh' }}>
      {/* App container. On desktop (lg+) this becomes a two-column card:
          the WizardRail on the left, the wizard itself in .fw-right. On
          mobile it stays the single sticky-header column (rail is hidden). */}
      <div className={`fw-container${embedded ? ' fw-embedded' : ''}`} style={{
        maxWidth: '900px',
        margin: '0 auto',
        background: 'white',
        minHeight: '100vh',
        ...
        {!isSuccess && (
          <WizardRail step={step} titles={STEP_TITLES} total={TOTAL_STEPS} />
        )}

        <div className="fw-right" style={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0 }}>

        {/* App header */}
        {!isSuccess && (
          <>
            <div className="fw-header" style={{
              ...
              {/* Web-only way out of the portal for public (farmer) visitors.
                  On mobile the farmer bottom tabs cover this; on desktop those
                  tabs are hidden, so without this a farmer has no exit here.
                  Admins are excluded: they arrive from the dashboard and use the
                  dashboard sidebar instead — sending them to the public landing
                  page would kick them out of the admin app. The header ← below
                  is step-back, not an exit. */}
              {!embedded && (
                <Link
                  href="/"
        ...
        {/* Content area */}
        <div className="fw-content" style={{ flex: 1, padding: '24px 20px' }}>
      ...
      {/* Bottom tabs on the welcome + success screens (no Back/Next bar there).
          Hidden when embedded in the dashboard — the admin sidebar is the nav
          there, and these farmer-facing tabs would collide with it. */}
      ...
      <style>{`
        ...
        /* Desktop (lg+) two-column web view. Mobile keeps the single sticky-
           header column above; these rules only kick in on wide screens. */
        .fw-rail { display: none; }
        @media (min-width: 1024px) {
          .fw-container {
            ...
            box-shadow: 0 24px 60px -24px rgba(16,25,11,0.28) !important;
          }
          /* Embedded in the dashboard there is no fixed public top nav to clear,
             so drop the tall top margin the standalone portal uses for it. */
          .fw-container.fw-embedded { margin: 32px auto 40px !important; }
          .fw-header { display: none !important; }
          .fw-rail { display: flex !important; }
          .fw-content { padding: 44px 48px !important; }
          .fw-bottom {
            position: static !important;
            justify-content: flex-end !important;
            padding: 20px 48px !important;
          }
          .fw-bottom button { flex: 0 0 auto !important; min-width: 150px; }
        }
      `}</style>
    </main>
  )
