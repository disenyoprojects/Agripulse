...
/**
 * The AI Price Advisor, shared by two routes:
 *  - `/price-advisor` — the public, farmer-facing page (standalone chrome).
 *  - `/dashboard/price-advisor` — the same tool embedded in the LGU dashboard
 *    shell (`embedded`), where the admin sidebar is the navigation. In that mode
 *    the public "Back" button, farmer bottom tabs, and the top padding that
 *    clears the public nav are dropped so the admin stays inside the dashboard.
 */
export default function PriceAdvisor({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <main style={{ background: '#f8f6ef', minHeight: '100vh', paddingTop: embedded ? 0 : '72px' }}>

      {/* Hero */}
      <section className="pa-hero" style={{
        background: 'radial-gradient(ellipse 60% 80% at 90% 10%, rgba(93,158,135,0.24) 0%, transparent 55%), linear-gradient(155deg, #2A5320 0%, #437628 100%)',
        ...
        <div style={{ maxWidth: '760px', position: 'relative' }}>
          {/* Public exit back to the site. Hidden when embedded in the dashboard
              — the admin sidebar is the navigation there. */}
          {!embedded && (
        ...
        {result && (
          <section style={{
            ...
            <div className="pa-price-hero" style={{
              background: 'linear-gradient(135deg, #437628, #2A5320)',
              borderRadius: '16px',
              padding: '28px 32px',
              color: 'white',
              display: 'flex',
      ...
      <footer style={{
        background: '#2A5320', color: '#d8e6c8',
      ...
      `}</style>
      {/* Farmer bottom tabs — hidden when embedded in the dashboard, where the
          admin sidebar is the navigation and these would collide with it. */}
      {!embedded && (
        <>
